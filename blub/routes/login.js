...
router.get('/', function(req, res, next) {
    if (!req.user) {
        res.render('login', { title: 'Remote ECC login', failureFlash: req.flash('error'), protocol: ((blubsetup.use_tls) ? 'wss://' : 'ws://'), server: blubsetup.host, endpoint: blubsetup.login_endpoint, mini_endpoint: blubsetup.login_endpoint });
    }
    else {
        res.redirect('/');
    }
